...
I'm very happy to report that our team has been working very hard to address these issues, and many others raised by users, to make using blogdown easier to learn and use. 
:rotating_light: tread here with caution :rotating_light:. 

In time for the new year when I know a lot of folks are itching to introduce themselves into online communities and conversations, I thought it would be fun to take my original blogdown post, and trace the same process of building a Hugo website with "modern blogdown" in 2021. I also wanted to add details about using the Hugo Academic theme (now named "Wowchemy"), based on questions I get during workshops and that I see on the RStudio Community forums.
...
{{% alert warning %}}
Always restart your R session after editing your `.Rprofile` for changes to take effect.
{{% /alert %}}
...
Let's go ahead and push our changes to GitHub. First, let's make a `.gitignore` file. It should look like this:
...
Thus far, we've only been previewing our site. Now, let's take a quick moment to *build* our site. 
...
This creates an actual new directory in your project called `public/`. This is why we just added that directory to our `.gitignore` file. We *can* build our site locally, but we'd rather let Netlify build it for us in the clouds as it improves your website quality of life. 

{{% alert note %}}
Note that you could publish the `public/` folder using Netlify [drag & drop](https://app.netlify.com/drop). Watch this [webinar](../talk/2020-sharing-short-notice/) called "Sharing on Short Notice" to learn more.
{{% /alert %}}

<video width="320" height="240" controls>
  <source src="https://rstudio-education.github.io/sharing-short-notice/images/drop-blog.mp4" type="video/mp4">
</video>

This workflow works, but I assume you want to ride this bike. Let's skip the training wheels &mdash; we are going one step more advanced by using GitHub with Netlify now.

You actually already have most of the wiring laid out for you. Our site has a `netlify.toml` file, which sets us the necessary settings for letting Netlify build our site for us and then publishing it. You can read mor about this file [here](/post/2019-02-19-hugo-netlify-toml/). 
...
Deploying in Netlify through GitHub is smooth. You can make it smoother by adding a `netlify.toml` file in the project root directory. I have a post with more details 
...
{{% alert note %}}
To get an `*.rbind.io` URL, follow [these instructions](/2017-06-12-up-and-running-with-blogdown/#rbindio-domain-names).
{{% /alert %}}
