...
2. Still in the RStudio console, load the `blogdown` package to run the `install_hugo` helper function:
...
{{% alert warning %}}
The most important thing to remember is to **not** knit your `.Rmd` posts. Use `blogdown::serve_site` instead. The only Netlify deployment failures I ever experienced were because I knitted to html.
{{% /alert %}}
...
**Note that anytime you change your subdomain name, you need to update the `baseurl` in your `config.toml` file (so I changed mine to `baseurl = "https://apreshill.netlify.com/"`).**
...
**Note that again, you will need to update the `baseurl` in your `config.toml` file to reflect your new rbind subdomain name (so mine is `baseurl = "https://apreshill.rbind.io/"`).**
...
<iframe src="https://giphy.com/embed/wORkexswPznJC" width="480" height="353" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="https://giphy.com/gifs/fun-shrek-donkey-wORkexswPznJC">via GIPHY</a></p>