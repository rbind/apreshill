...
Having `blogdown::serve_site` running locally with LiveReload is especially useful as you can immediately see if you have totally screwed up. For example, in adding to my `about.md` file, I saw this error pop up in my console after making a change and I was able to fix the error right away:
```{r}
...
Use the Addin for `New Post`. But, you need the console to do this, so you have to stop `blogdown::serve_site`. The Addin is a Shiny interface that runs this code in your console: `blogdown:::new_post_addin()`. So, your console needs to be unblocked for it to run. You also need to be "in" your RStudio project or it won't work.
...
Either way, this may seem obvious, but when you want to edit your `.Rmd` posts, in addition to saving the changes in your `.Rmd` file, you *must* use `blogdown::serve_site`- this is how the output `html` file is generated.
