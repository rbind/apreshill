...
1. Go online to your [GitHub](https://github.com) account, and create a new repository (check to initialize with a README but don't add a gitignore- this will be taken care of later). For naming your repo, consider your future deployment plan:
...
{{% alert note %}}
These installations are just like installing packages in R - your working directory does not matter for either of these two steps, so don't worry about where R thinks you are.
{{% /alert %}}
...
Listen to Yihui and Amber- they offer ["to save you some time, we list a few themes below that match our taste..."](https://bookdown.org/yihui/blogdown/other-themes.html). I went with [hugo-academic](https://github.com/gcushen/hugo-academic). Whatever theme you choose, you'll need to pick one of 3 ways to make your new site:
...
{{% alert warning %}}
If your example site is not loading correctly in your RStudio viewer pane, I have found that several themes replace the baseurl with something like "https://example.com", without the trailing `/`. This is an infuriating but quick fix- see below section on editing configurations.
{{% /alert %}}
...
* View [my `config.toml` file](https://github.com/apreshill/apreshill/blob/master/config.toml)

1. Edit the `baseurl` in your `config.toml` file. The URL *should always* end with a `/` trailing slash. At this point, you probably haven't deployed your site yet, so to view it locally you can use the Addin "Serve Site", or run the `blogdown::serve_site` function.
...
Addins: use them. Note that you don't need the `blogdown` package loaded if you use the Addins.

Relevant reading:

* [`blogdown` book chapter on the RStudio IDE](https://bookdown.org/yihui/blogdown/rstudio-ide.html)

My workflow in RStudio at this point works best like this:

1. Open the RStudio project for my website
2. Use the add-in to serve the site (you only need to do this once due to LiveReload), which if all goes well appears in the RStudio viewer pane. I show it in a new brower window and just leave it open there.
3. Select existing files to work on through the file pane in RStudio, or use add-ins to create new files (see below).
4. After making changes, click the save button- the console will reload, the viewer pane will update, and if you hit refresh in the browser your local view will also be updated.
...
This was surprisingly confusing to me at first. Here are some relevant links:
...
2. Click on the **Sign Up** button and sign up using your existing GitHub account. 
...
At this point, you should be up and running with `blogdown`, GitHub, and Netlify, but here are some final things to play with...

# Custom CSS

I like to tinker with theme settings like colors, fonts, etc. Every Hugo theme is structured a little differently, but if you are interested, you can check out [my custom css](https://github.com/apreshill/apreshill/blob/master/static/css/blue.css) to see how I tinkered with the academic theme, which provides this option in the `config.toml` file:
