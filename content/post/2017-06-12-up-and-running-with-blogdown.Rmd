...
This may seem obvious, but when you edit your `.Rmd` posts, in addition to saving the changes in your `.Rmd` file, you *must* use `blogdown::serve_site`- this is how the output `html` file is generated.
...
Ultimately, your [YAML metadata looks like this](https://bookdown.org/yihui/blogdown/output-format.html#output-format):
...
So, although `blogdown::html_page` inherits from `bookdown::html_document2` which inherits from `rmarkdown::html_document`, you cannot knit an `.Rmd` file into an `.html`.  
